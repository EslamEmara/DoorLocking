...
void keypad_init(){
  ...
  DIO_SetPinDirection(PORT_C,4,INPUT);



  DIO_SetPinDirection(PORT_C,5,INPUT);



  DIO_SetPinDirection(PORT_C,6,INPUT);
...
uint8_t keypad_read(){
    keypad_line_scan();
    if((PORTC & 0b01110000) != 0){
        uint8_t keyValue = keypad_value(PORTC);
        return keyValue;
    }
    return 0;
}
